...
nerdctl run -it --net=host --rm -v ${PWD}:/models tritonserver:${triton_version}-py3-sdk-tchtf /bin/bash
  cd /models

nerdctl start `nerdctl ps -a | grep "tritonserver:${triton_version}-py3-sdk-tchtf" | awk '{print $1}'`