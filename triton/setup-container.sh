...
nerdctl exec -it `nerdctl ps -a | grep tritonserver:22.04-py3 | awk '{print $1}'` /bin/bash
  cd /models
  pip install torch-1.12.0+cu116-cp38-cp38-linux_x86_64.whl
  pip install torchvision-0.13.0+cu116-cp38-cp38-linux_x86_64.whl
nerdctl commit `nerdctl ps -a | grep tritonserver:22.04-py3 | awk '{print $1}'` tritonserver:22.04-py3-tchtf
nerdctl run --gpus all --rm -p8000:8000 -p8001:8001 -p8002:8002 --net=host --rm -v ${PWD}:/models tritonserver:22.04-py3-tchtf tritonserver --model-repository=/models
...
nerdctl start `nerdctl ps -a | grep "tritonserver:22.04-py3-sdk-tchtf" | awk '{print $1}'`